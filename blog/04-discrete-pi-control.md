...
> 本章提供 Simulink 离散 PI 平均模型、Python 可复现仿真脚本、CSV 原始数据和波形图。正文中的控制波形由脚本生成，不是手工画出来的示意结果。
...
![P-only 和 PI 在输入电压阶跃下的对比](../waveforms/04-p-only-vs-pi-vin-step.png)
...
![离散 PI 在输入扰动和负载扰动下的恢复过程](../waveforms/04-pi-vin-load-transient.png)
...
| 输入阶跃后 Vout 最低/最高 | 约 10.26V / 12.29V | 这组参数有明显欠阻尼，不是最终调参 |
| 负载阶跃后 Vout 最低/最高 | 约 11.25V / 12.66V | 恢复可见，但仍需后续优化 |
| duty 范围 | 约 0.504 - 0.648 | 未触及 1，但本章还没加限幅 |
| 输入阶跃 1% 恢复时间 | 约 2.41ms | 从 3ms 扰动点开始计算 |
| 负载阶跃 1% 恢复时间 | 约 1.01ms | 从 7ms 扰动点开始计算 |
...
![PI 误差和积分项变化](../waveforms/04-pi-error-integrator.png)
...
![离散控制器采样点](../waveforms/04-pi-sampling-points.png)
...
| duty 没有限幅 | 本章 duty 最大约 0.648，但代码没有上限保护 | 第五章处理 |
...
| Simulink 截图脚本 | `scripts/export_simulink_discrete_pi_snapshot.m` | 生成模型和截图 |
| Python 仿真脚本 | `scripts/export_discrete_pi_control.py` | 生成 CSV 数据和波形图 |
| 原始数据 | `waveforms/04-discrete-pi-control-trace.csv` | PI 仿真时序数据 |
| 指标汇总 | `waveforms/04-discrete-pi-control-summary.csv` | 本章表格中的关键指标 |
| 控制波形 | `waveforms/04-*.png` | 本章使用的仿真波形 |
...
如果 MATLAB 没有加入系统 PATH，可以把第二条命令里的 `matlab` 替换成你本机 MATLAB 的完整路径。
