...
原因是第 10 章的测试契约只包含 C 风格接口和 Python 同等算法测试台，没有执行 C 编译，也没有接入 ADC、PWM、定时器和寄存器驱动。电脑端 C 编译与基础单元测试由第 11 章继续完成。
...
第 10 章先提供 C 风格源码和同等算法测试台，没有把 C 编译纳入本章验证范围。第 11 章随后使用 Zig 编译真实 C 控制器并运行电脑端基础单元测试。
